...
# ----------------------------------------------------
# 8. 제작 국가별 총 관객 수 비교 (막대 그래프)
# ----------------------------------------------------
st.subheader("8. 제작 국가별 총 관객 수 비교")

# 국가별 총 관객 수 합계 및 평균, 개봉 편수 집계
nation_stats = (
    df.groupby("nation")
    .agg(
        total_audi_sum=("total_audi", "sum"),
        total_audi_mean=("total_audi", "mean"),
        movie_count=("movieCd", "count"),
    )
    .reset_index()
    .sort_values(by="total_audi_sum", ascending=False)
)

# 총 관객 수가 가장 많은 국가 및 수치 자동 계산
top_nation = nation_stats.iloc[0]
top_nation_name = top_nation["nation"]
top_nation_sum = top_nation["total_audi_sum"]
...
fig8 = px.bar(
    ...
    y="total_audi_sum",
    color="nation",
    title="제작 국가별 총 관객 수 합계",
    labels={
        "nation": "제작 국가",
        "total_audi_sum": "총 관객 수 합계",
        "movie_count": "개봉 편수",
    },
    hover_data={
        "total_audi_sum": ":,f",
...
fig8.update_traces(
    hovertemplate="<b>%{x}</b><br>총 관객 수 합계: %{y:,.0f}명<br>개봉 편수: %{customdata[2]}편<br>편당 평균 관객 수: %{customdata[1]:,.0f}명"
)
...
# 예시: key 옵션 추가
st.plotly_chart(fig1, use_container_width=True, key="chart_genre_pie")
